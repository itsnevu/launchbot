...
const initial = () => ({ step: null, platform: null, data: {}, pk: null, busy: false, ownerId: null, editing: false, lastActive: Date.now() });
...
const reset = (s) => { if (s.pk) forgetSecret(s.pk); s.step = null; s.platform = null; s.data = {}; s.pk = null; s.busy = false; s.ownerId = null; s.editing = false; };
...
const fmtResult = (platform, s, res) => {
  ...
  return `${res.dryRun ? "🔎 SIMULASI OK" : "✅ LAUNCHED"} di <b>${esc(platform.label)}</b>\n\n` +
    ...
    `Dev buy: ${esc(res.devBuy)} ${platform.nativeSymbol} → ${res.dryRun ? "≈" : "~"}${esc(Number(res.tokensOut).toLocaleString("en-US", { maximumFractionDigits: 2 }))} ${esc(s.data.symbol)}\n` +
